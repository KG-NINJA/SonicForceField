...
Repository: `KG-NINJA/SonicForceField`.

For setup or project-specific behavior, consult `README.md`.

Existing validation/build commands (use only those relevant to the change):

- `npm run test` (defined in `package.json`)
- `npm run check` (defined in `package.json`)
- `npm run lint` (defined in `package.json`)
- `npm run typecheck` (defined in `package.json`)
- `npm run build` (defined in `package.json`)

Existing CI/deployment definitions: `.github/workflows/pages.yml`. Inspect applicable triggers before push/merge. A workflow filename alone does not prove a deploy target or successful deployment.
